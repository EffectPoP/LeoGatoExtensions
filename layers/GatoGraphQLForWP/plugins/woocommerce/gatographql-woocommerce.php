<?php
/*
Plugin Name: Gato GraphQL - WooCommerce
Plugin URI:
Description: WooCommerce extension for Gato GraphQL
Version: 1.1.0-dev
...
License:
License URI:
Text Domain: gatographql-woocommerce
...
\add_action('init', function (): void {
    load_plugin_textdomain('gatographql-woocommerce', false, plugin_basename(__FILE__) . '/languages');
});
...
add_action(
    ...
    function (): void {
        ...
        $extensionName = \__('Gato GraphQL - WooCommerce', 'gatographql-woocommerce');
        /**
         * @gatographql-extension-info
         *
        ...
        if (!class_exists(Plugin::class)) {
            \add_action('admin_notices', function () use ($extensionName) {
                printf(
                    ...
                    sprintf(
                        __('Plugin <strong>%s</strong> is not installed or activated. Without it, plugin <strong>%s</strong> will not be loaded.', 'gatographql-woocommerce'),
                        __('Gato GraphQL', 'gatographql-woocommerce'),
                        $extensionName
                    )
                );
            });
        ...
        if (!$extensionManager->isExtensionBundled(GatoGraphQLExtension::class))  {
             ...
             * Yoast SEO or, in this case, WooCommerce), add below:
             * 
             ...
             * ------------------------------------------------------------
             * 
             * Validate the WooCommerce plugin is active and satisfy
            ...
            if ($requiredPluginFile !== '') {
                ...
                if (!$isWordPressPluginActive
                    ...
                    \add_action('admin_notices', function () use ($extensionName, $isWordPressPluginActive, $requiredPluginVersion) {
                         ...
                         * 
                         * If the extension is an integration for some plugin (eg: WooCommerce,
                         * Yoast SEO or, in this case, WooCommerce), indicate the plugin's name:
                         */
                        $requiredPluginName = __('WooCommerce', 'gatographql-woocommerce');
                        printf(
                            '<div class="notice notice-error"><p>%s</p></div>',
                            $isWordPressPluginActive
                                ? sprintf(
                                    __('Installed version of plugin <strong>%s</strong> does not satisfy required constraint <code>%s</code>. Plugin <strong>%s</strong> has not been loaded.', 'gatographql-woocommerce'),
                                    $requiredPluginName,
                                    $requiredPluginVersion,
                                    $extensionName
                                )
