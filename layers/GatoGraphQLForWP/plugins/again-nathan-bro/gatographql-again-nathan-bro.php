<?php
/*
Plugin Name: Gato GraphQL - Again Nathan Bro
Plugin URI:
...
License:
License URI:
Text Domain: gatographql-again-nathan-bro
...
\add_action('init', function (): void {
    load_plugin_textdomain('gatographql-again-nathan-bro', false, plugin_basename(__FILE__) . '/languages');
});
...
add_action(
    ...
    function (): void {
        ...
        $extensionName = \__('Gato GraphQL - Again Nathan Bro', 'gatographql-again-nathan-bro');
        /**
         * @gatographql-extension-info
         *
        ...
        if (!class_exists(Plugin::class)) {
            \add_action('admin_notices', function () use ($extensionName) {
                printf(
                    ...
                    sprintf(
                        __('Plugin <strong>%s</strong> is not installed or activated. Without it, plugin <strong>%s</strong> will not be loaded.', 'gatographql-again-nathan-bro'),
                        __('Gato GraphQL', 'gatographql-again-nathan-bro'),
                        $extensionName
                    )
                );
            });
        ...
        if (!$extensionManager->isExtensionBundled(GatoGraphQLExtension::class))  {
             ...
             * ------------------------------------------------------------
             * 
             * Validate the Again Nathan Bro plugin is active and satisfy
            ...
            if ($requiredPluginFile !== '') {
                ...
                if (!$isWordPressPluginActive
                    ...
                    \add_action('admin_notices', function () use ($extensionName, $isWordPressPluginActive, $requiredPluginVersion) {
                         ...
                         * 
                         * If the extension is an integration for some plugin (eg: WooCommerce,
                         * Yoast SEO or, in this case, Again Nathan Bro), indicate the plugin's name:
                         */
                        ...
                        printf(
                            ...
                            $isWordPressPluginActive
                                ? sprintf(
                                    __('Installed version of plugin <strong>%s</strong> does not satisfy required constraint <code>%s</code>. Plugin <strong>%s</strong> has not been loaded.', 'gatographql-again-nathan-bro'),
                                    $requiredPluginName,
                                    $requiredPluginVersion,
                                    $extensionName
                                )
