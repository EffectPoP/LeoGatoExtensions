<?php
/*
Plugin Name: Gato GraphQL - Nathan Bro
Plugin URI:
Description: Nathan Bro extension for Gato GraphQL
Version: 1.1.0-dev
...
License:
License URI:
Text Domain: gatographql-nathan-bro
...
\add_action('init', function (): void {
    load_plugin_textdomain('gatographql-nathan-bro', false, plugin_basename(__FILE__) . '/languages');
});
...
add_action(
    ...
    function (): void {
        ...
        $extensionName = \__('Gato GraphQL - Nathan Bro', 'gatographql-nathan-bro');
        /**
         * @gatographql-extension-info
         *
        ...
        if (!class_exists(Plugin::class)) {
            \add_action('admin_notices', function () use ($extensionName) {
                printf(
                    ...
                    sprintf(
                        __('Plugin <strong>%s</strong> is not installed or activated. Without it, plugin <strong>%s</strong> will not be loaded.', 'gatographql-nathan-bro'),
                        __('Gato GraphQL', 'gatographql-nathan-bro'),
                        $extensionName
                    )
                );
            });
        ...
        if (!$extensionManager->isExtensionBundled(GatoGraphQLExtension::class))  {
             ...
             * ------------------------------------------------------------
             * 
             * Validate the Nathan Bro plugin is active and satisfy
            ...
            if ($requiredPluginFile !== '') {
                ...
                if (!$isWordPressPluginActive
                    ...
                    \add_action('admin_notices', function () use ($extensionName, $isWordPressPluginActive, $requiredPluginVersion) {
                         ...
                         * 
                         * If the extension is an integration for some plugin (eg: WooCommerce,
                         * Yoast SEO or, in this case, Nathan Bro), indicate the plugin's name:
                         */
                        ...
                        printf(
                            ...
                            $isWordPressPluginActive
                                ? sprintf(
                                    __('Installed version of plugin <strong>%s</strong> does not satisfy required constraint <code>%s</code>. Plugin <strong>%s</strong> has not been loaded.', 'gatographql-nathan-bro'),
                                    $requiredPluginName,
                                    $requiredPluginVersion,
                                    $extensionName
                                )
